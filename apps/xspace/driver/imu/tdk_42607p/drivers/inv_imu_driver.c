                 ...
                 void (*sensor_event_cb)(inv_imu_sensor_event_t *event))
{
    ...
    inv_imu_sleep_us(30000);

    if ((status |= configure_serial_interface(s)) != 0){
        INV_MSG(INV_MSG_LEVEL_INFO, "%d",status);
        return status;
    }
...
int inv_imu_get_config_int2(struct inv_imu_device *s, inv_imu_interrupt_parameter_t *it)
{
    ...
    it->INV_STEP_CNT_OVFL = (inv_imu_interrupt_value)((data & INT_SOURCE7_STEP_CNT_OFL_INT2_EN_MASK)
...
int inv_imu_configure_fifo(struct inv_imu_device *s, INV_IMU_FIFO_CONFIG_t fifo_config)
{
    ...
    switch (fifo_config) {
		...
		*/
            ...
            data |=
                (uint8_t)FIFO_CONFIG1_FIFO_MODE_SNAPSHOT | (uint8_t)FIFO_CONFIG1_FIFO_BYPASS_OFF;
...
static int init_hardware_from_ui(struct inv_imu_device *s)
{
    ...
    value |= (uint8_t)INT_CONFIG_INT2_POLARITY_LOW;
    status |= inv_imu_write_reg(s, INT_CONFIG, 1, &value);

//Note(Mark):config mode for INT2 to INT_CONFIG_INT2_MODE_PULSED
    status |= inv_imu_read_reg(s, INT_CONFIG, 1, &value);
    value &= ~INT_CONFIG_INT2_MODE_MASK;
    value |= (uint8_t)INT_CONFIG_INT2_MODE_PULSED;
    status |= inv_imu_write_reg(s, INT_CONFIG, 1, &value);
