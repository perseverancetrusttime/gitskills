...
#define RFCOMM_SERVER_MAX_CHANNEL_CNT    6
...
void *app_rfcomm_get_spp_dev(uint8_t index)
{
    return (void *)(RfcommServiceInstance[index].sppDev);
}


static void app_rfcomm_org_callback(uint8_t device_id, struct spp_device *locDev, struct spp_callback_parms *info)
{
    ...
    TRACE(4,"%s %p %d %d",__func__, locDev, index, device_id);
    TRACE(2,"%s %p 0x%0x",__func__, pService->callback,btif_me_get_remote_device_hci_handle(info->p.remDev));
    ...
    if (info->p.remDev)
    {
        ...
        {
            bool ret = pService->callback(device_id,
                                          event,
                                          index,
            ...
            if (!ret)
            {
                ...
                if(RFCOMM_CONNECTED == event)
                {
                    app_rfcomm_disconnect((void *)locDev, BTIF_BEC_BT_DISCONNECT_ITSELF);
                }
            }
        }
    }
...
static bt_status_t app_rfcomm_service_init(RfcommService_t *service,
                                ...
                                int attributeCount, int serviceId, uint64_t app_id,
                                spp_handle_data_event_func_t dataRecCallback,
                                int txPacketCount,
                                osMutexId mutexId,
                                osMutexId creditMutexId) {

    struct spp_service *sppService = btif_create_spp_service();
    btif_sdp_record_t *sppSdpRecord = NULL;
    ...
    btif_spp_service_setup(service->sppDev, sppService, sppSdpRecord);
...
bool app_rfcomm_removesdpservice(uint8_t instanceIndex)
{
    ...
    return (BT_STS_SUCCESS == btif_sdp_remove_record(&(RfcommServiceInstance[instanceIndex].sppDev->sppDev.type.sppService->sdpRecord)));
}
...
bool app_rfcomm_addsdpservice(uint8_t instanceIndex)
{
    ...
    return (BT_STS_SUCCESS == btif_sdp_add_record(&(RfcommServiceInstance[instanceIndex].sppDev->sppDev.type.sppService->sdpRecord)));
}
...
void app_rfcomm_disconnect(void *spp_dev, uint8_t reason)
{
    spp_device *dev = (spp_device *)spp_dev;

    if (dev != NULL)
    {
        TRACE(3,"%s,spp_dev=%p,reason=%d", __func__, dev, reason);
        btif_spp_disconnect(dev, reason);
    }
}
...
int8_t app_rfcomm_open(RFCOMM_CONFIG_T* ptConfig)
{
    ...
    sdp_attribute_t *sdpAtt;

    sppServiceInstance->callback = ptConfig->callback;

    for(uint8_t i = 0; i < rfcomm_service_used_instance_cnt; i++)
    {
        TRACE(5,"%s %d %p %d %d",__func__,i,rfcommAttributesArray[i][1].value,rfcommAttributesArray[i][1].value[13],ptConfig->rfcomm_ch);
        if(rfcommAttributesArray[i][1].value[13] == ptConfig->rfcomm_ch)
        {
            sdp_add = false;
            sdpAtt = rfcommAttributesArray[i];
            sppServiceInstance->instanceIndex = i;
            break;
        }
     };
    ...
    if(sdp_add) {
        ...
        sdpAtt = rfcommAttributesArray[rfcomm_service_used_instance_cnt];
    }

    bt_status_t status = app_rfcomm_service_init(
      sppServiceInstance, sdpAtt,
