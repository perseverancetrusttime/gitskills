...
bool audio_prompt_clear_pending_stream(uint8_t op)
{
    ...
    TRACE(4, "%s stop_id %d pendingStopOp %d op %d",
            __func__,
...
void audio_prompt_stop_playing(void)
{
    ...
    lock = int_lock_global();
    op_status = app_audio_list_rmv_callback(&aud_status, &status_next,APP_BT_SETTING_Q_POS_HEAD);
    int_unlock_global(lock);
...
void app_tws_cmd_prompt_play_req_handler(uint8_t *ptrParam, uint32_t paramLen)
{
    ...
    voice_report_role_t report_role =  app_ibrt_voice_report_get_role();
    ptPlayReq->dev_id = bt_media_get_active_device_by_type(BT_STREAM_SBC);

    if(ptPlayReq->dev_id == BT_DEVICE_INVALID_ID)
    {
        uint8_t dev_id = bt_media_get_active_device_by_type(BT_STREAM_VOICE);
        if(dev_id == BT_DEVICE_INVALID_ID)
        {
            dev_id = BT_DEVICE_ID_1;//default BT_DEVICE_ID_1
        }
        ptPlayReq->dev_id = dev_id;
    }
