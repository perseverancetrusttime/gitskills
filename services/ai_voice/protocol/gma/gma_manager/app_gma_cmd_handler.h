...
void                        app_gma_start_data_xfer_control_rsp_handler(APP_GMA_CMD_RET_STATUS_E retStatus, uint8_t* ptrParam, uint32_t paramLen, uint8_t device_id);
void                        app_gma_stop_data_xfer_control_rsp_handler(APP_GMA_CMD_RET_STATUS_E retStatus, uint8_t* ptrParam, uint32_t paramLen, uint8_t device_id);
APP_GMA_CMD_RET_STATUS_E    app_gma_send_response_to_command(APP_GMA_CMD_CODE_E funcCode, uint8_t* ptrParam, uint32_t paramLen, uint8_t device_id);
APP_GMA_CMD_RET_STATUS_E    app_gma_send_command(APP_GMA_CMD_CODE_E cmdCode, uint8_t* ptrParam, uint32_t paramLen, bool isCmd, uint8_t device_id);
