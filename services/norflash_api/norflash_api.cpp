...
static void _flash_remap_start(enum HAL_FLASH_ID_T id,uint32_t addr, uint32_t len)
{
    ...
    if(!hal_norflash_get_remap_status(id))
    {
        ...
        ret = hal_norflash_enable_remap(id);
...
static void _flash_remap_config(enum HAL_FLASH_ID_T id)
{
    ...
    {
        // NORFLASH_API_TRACE(3,"%s: Unremap to enable remap.",__func__);
        ret = hal_norflash_config_remap(id, remap_addr, remap_len, remap_offset);
        ASSERT(ret == HAL_NORFLASH_OK, "%s: Failed to config remap(0x%x,0x%x,0x%x), ret = %d",
...
#endif
#endif
    FLASH_REMAP_CONFIG(HAL_FLASH_ID_0);
