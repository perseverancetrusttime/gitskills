...
template <typename bias_t>
void beco_add_bias_16x32(const bias_t *bias, p31_t q)
{
    uint32_t config;
    uint32_t a0, a1;
    const uint32_t *p_bias = (const uint32_t *)bias;
    ...
    beco_write_reg(BECO_REG2, ((beco_vec64_in_t){.u32 = {a0, a1}}));
    beco_mmacgr(BECO_ACC0, ((beco_vec32_in_t){.u32 = q}), BECO_REG2);
...
template void beco_add_bias_16x32<>(const int8_t *bias, p31_t q);
template void beco_add_bias_16x32<>(const uint8_t *bias, p31_t q);
template void beco_add_bias_16x32<>(const int16_t *bias, p31_t q);
template void beco_add_bias_16x32<>(const uint16_t *bias, p31_t q);
