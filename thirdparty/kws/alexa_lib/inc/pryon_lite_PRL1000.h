...
///
DLLDECL PryonLiteError PryonLite_GetConfigAttributes(const PryonLiteV2Config* config, PryonLiteV2EventConfig* eventConfig, PryonLiteV2ConfigAttributes* configAttributes);
...
/// @return PRYON_LITE_ERROR_OK if successful, otherwise a non-zero error code
///
DLLDECL PryonLiteError PryonLite_Initialize(const PryonLiteV2Config* config, PryonLiteV2Handle* handle, PryonLiteEvent_Callback publicEventCallback, PryonLiteV2EventConfig* eventConfig, char* engineMem, size_t sizeofEngineMem);
...
/// @return PRYON_LITE_ERROR_OK if successful, otherwise a non-zero error code
///
DLLDECL PryonLiteError PryonLite_Destroy(PryonLiteV2Handle *handle);
...
/// @return PRYON_LITE_ERROR_OK if successful, otherwise a non-zero error code
///
DLLDECL PryonLiteError PryonLite_PushAudioSamples(PryonLiteV2Handle *handle, const short *samples, int sampleCount);
