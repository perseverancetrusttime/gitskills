...
int main(int argc, char **argv)
{
    ...
    PryonLiteError status = PryonLite_GetModelAttributes(config.model, config.sizeofModel, &modelAttributes);
    ...
    while(1)
    {
        ...
        status = PryonLiteDecoder_PushAudioSamples(sDecoder, samples, sessionInfo.samplesPerFrame);
